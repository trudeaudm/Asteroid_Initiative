	...
	public override void OnInspector()
	{
		...
		SGT_EditorGUI.BeginGroup("Surface");
		{
			...
			Target.SurfaceRenderQueue   = SGT_EditorGUI.IntField("Render Queue", "The render queue used by the surface mesh.", Target.SurfaceRenderQueue);
			                              SGT_EditorGUI.SurfaceMultiMeshField("Mesh", "This should be a sphere with a radius of 1.", Target.SurfaceMesh, true);
			                              SGT_EditorGUI.Field("Texture", "This is the texture used by the star's surface.", Target.SurfaceTexture, true);
		...
		SGT_EditorGUI.BeginGroup("Atmosphere");
		{
			...
			Target.AtmosphereHeight        = SGT_EditorGUI.FloatField("Height", "Distance between the surface of the star and the top of the atmosphere.", Target.AtmosphereHeight);
			                                 SGT_EditorGUI.Field("Density Colour", "The colour of the atmosphere based on the optical thickness. Left = Star's centre. Centre = Horizon. Right = Sky's zenith.", Target.AtmosphereDensityColour);
