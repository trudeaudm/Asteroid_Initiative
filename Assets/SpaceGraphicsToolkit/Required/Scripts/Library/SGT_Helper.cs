...
public static class SGT_Helper
{
	...
	public static float Wrap(float current, float min, float max) // [min .. max)
    {
		var x = Mod(current - min, max - min);
		
		return x < 0.0f ? max + x : min + x;
	}
	
	public static float Delta(float current, float target, float min, float max)
	{
		current = Wrap(current, min, max);
		target  = Wrap(target, min, max);
