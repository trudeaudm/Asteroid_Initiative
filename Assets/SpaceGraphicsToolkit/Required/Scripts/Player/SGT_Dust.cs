...
public class SGT_Dust : SGT_MonoBehaviourUnique<SGT_Dust>
{
	...
	private void Validate()
	{
		...
		if (dustCamera   == null) DustCamera   = SGT_Helper.FindCamera(); // NOTE: Assigning property
		...
		var oldCameraRotation   = dustCameraRotation; if (dustCamera != null) dustCameraRotation = dustCamera.transform.rotation;
		var cameraRotationDelta = Quaternion.Inverse(oldCameraRotation) * dustCameraRotation;
		
		dustCameraRoll = SGT_Helper.Wrap(dustCameraRoll - cameraRotationDelta.eulerAngles.z, 0.0f, 360.0f);
		
		var roll = Quaternion.Euler(new Vector3(0.0f, 0.0f, dustCameraRoll));
		...
		dustMaterial.SetMatrix("cameraRoll", SGT_MatrixHelper.Rotation(roll));
